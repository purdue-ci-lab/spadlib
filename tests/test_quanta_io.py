...
def test_zarr_roundtrip(zarr_path, frames):
    read_frames, (t, y, x), attrs = read_quanta_zarr(zarr_path, load_data=True)
    _check(read_frames, frames)
    assert attrs["T"] == T and attrs["H"] == H and attrs["W"] == W


def test_bin_roundtrip(bin_path, frames):
    _check(read_quanta_bin(bin_path, H=H, W=W), frames)


def test_mat_roundtrip_autokey(mat_path, frames):
    _check(read_quanta_mat(mat_path), frames)


def test_mat_roundtrip_explicit_key(mat_path, frames):
    _check(read_quanta_mat(mat_path, key=MAT_KEY), frames)


# --- directory formats ----------------------------------------------------
def test_bin_dir_roundtrip(bin_dir, frames):
    _check(read_quanta_dir(bin_dir, H=H, W=W), frames)


def test_mat_dir_roundtrip(mat_dir, frames):
    _check(read_quanta_dir(mat_dir), frames)


# --- auto-detection dispatch ----------------------------------------------
def test_auto_zarr(zarr_path, frames):
    read_frames, meta = read_quanta_auto(zarr_path, load_data=True)
    _check(read_frames, frames)
    assert meta["T"] == T


def test_auto_bin(bin_path, frames):
    read_frames, meta = read_quanta_auto(bin_path, H=H, W=W)
    _check(read_frames, frames)
    assert meta is None


def test_auto_mat(mat_path, frames):
    read_frames, meta = read_quanta_auto(mat_path)
    _check(read_frames, frames)


def test_auto_bin_dir(bin_dir, frames):
    read_frames, meta = read_quanta_auto(bin_dir, H=H, W=W)
    _check(read_frames, frames)


def test_auto_mat_dir(mat_dir, frames):
    read_frames, meta = read_quanta_auto(mat_dir)
    _check(read_frames, frames)
